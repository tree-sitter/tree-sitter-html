...
static unsigned serialize(Scanner *scanner, char *buffer) {
    ...
    for (; serialized_tag_count < tag_count; serialized_tag_count++) {
        Tag *tag = array_get(&scanner->tags, serialized_tag_count);
        if (tag->type == CUSTOM) {
...
static void deserialize(Scanner *scanner, const char *buffer, unsigned length) {
    for (unsigned i = 0; i < scanner->tags.size; i++) {
        tag_delete(array_get(&scanner->tags, i));
    }
        ...
        if (tag_count > 0) {
            ...
            for (iter = 0; iter < serialized_tag_count; iter++) {
                ...
                if (tag.type == CUSTOM) {
                    ...
                    tag.custom_tag_name.capacity = name_length;
                    array_reserve(&tag.custom_tag_name, name_length + 1);
...
static String scan_tag_name(TSLexer *lexer) {
    ...
    array_reserve(&tag_name, 17);
    while (iswalnum(lexer->lookahead) || lexer->lookahead == '-' || lexer->lookahead == ':') {
        array_push(&tag_name, towupper(lexer->lookahead));
        advance(lexer);
    }
    array_push(&tag_name, '\0');
    return tag_name;
}
...
static bool scan_implicit_end_tag(Scanner *scanner, TSLexer *lexer) {
    ...
    if (lexer->lookahead == '/') {
        ...
        if (parent && is_void(parent)) {
            tag_delete(&array_pop(&scanner->tags));
    ...
    Tag next_tag = for_name(tag_name);

    if (is_closing_tag) {
        // The tag correctly closes the topmost element on the stack
        if (scanner->tags.size > 0 && tagcmp(array_back(&scanner->tags), &next_tag)) {
            array_delete(&tag_name);
            tag_delete(&next_tag);
            return false;
        }
        ...
        for (unsigned i = scanner->tags.size; i > 0; i--) {
            if (array_get(&scanner->tags, i - 1)->type == next_tag.type) {
                tag_delete(&array_pop(&scanner->tags));
                lexer->result_symbol = IMPLICIT_END_TAG;
                array_delete(&tag_name);
                tag_delete(&next_tag);
...
static bool scan_end_tag_name(Scanner *scanner, TSLexer *lexer) {
    ...
    Tag tag = for_name(tag_name);
    if (scanner->tags.size > 0 && tagcmp(array_back(&scanner->tags), &tag)) {
        tag_delete(&array_pop(&scanner->tags));
